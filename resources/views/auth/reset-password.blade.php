<x-guest-layout>
    <div class="w-full max-w-md mx-4 sm:mx-auto arcade-frame bg-gradient-to-br from-fuchsia-900 via-purple-900 to-fuchsia-900 border-4 border-fuchsia-300/80 rounded-t-[2rem] rounded-b-md shadow-[0_0_30px_10px_rgba(236,72,153,0.7),0_0_50px_15px_rgba(168,85,247,0.5)] relative overflow-hidden neon-glow">
        ...
        <div class="bg-black p-4 rounded-t-[2rem] border-b-4 border-fuchsia-400 text-center shadow-inner relative overflow-hidden">
            ...
            <h2 class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-300 to-purple-300 tracking-wider neon-text">REESTABLECER CONTRASEÑA</h2>
        </div>

        <!-- Formulario -->
        <form method="POST" action="{{ route('password.store') }}" class="p-4 sm:p-6 md:p-8 space-y-4 text-white relative z-10">
            @csrf
            ...
            <!-- Password -->
            <div class="relative group">
                <x-input-label for="password" :value="__('Nueva Contraseña')" class="text-purple-200 text-sm sm:text-base font-mono tracking-wide" />
            ...
            <!-- Botón -->
            <div class="mt-6">
                <x-primary-button class="relative w-full overflow-hidden bg-gradient-to-r from-fuchsia-500 to-purple-600 hover:from-fuchsia-400 hover:to-purple-500 text-white font-bold px-6 py-2 rounded-lg shadow-[0_0_20px_5px_rgba(236,72,153,0.5)] transform transition-all hover:scale-105 group text-sm sm:text-base">
                    <span class="relative z-10 font-mono tracking-wider">{{ __('REESTABLECER CONTRASEÑA') }}</span>

    ...
    <div class="w-full max-w-md mx-4 sm:mx-auto arcade-frame bg-gradient-to-br from-fuchsia-900 via-purple-900 to-fuchsia-900 border-4 border-fuchsia-300/80 rounded-t-[2rem] rounded-b-md shadow-[0_0_30px_10px_rgba(236,72,153,0.7),0_0_50px_15px_rgba(168,85,247,0.5)] relative overflow-hidden neon-glow">
        ...
        <div class="p-4 sm:p-6 md:p-8 text-white text-sm sm:text-base font-mono space-y-4 relative z-10">
            <p class="text-fuchsia-200 leading-relaxed tracking-wide">
                {{ __('Gracias por registrarte. Antes de comenzar, verifica tu dirección de correo electrónico haciendo clic en el enlace que acabamos de enviarte. Si no lo recibiste, con gusto te lo enviaremos nuevamente.') }}
            </p>

            @if (session('status') == 'verification-link-sent')
                <div class="text-green-400 font-semibold">
                    {{ __('Se ha enviado un nuevo enlace de verificación al correo que proporcionaste.') }}
                </div>
            @endif
            ...
            <div class="mt-6 flex flex-col sm:flex-row sm:justify-between gap-4">
                <form method="POST" action="{{ route('verification.send') }}">
                    @csrf
                    <x-primary-button class="w-full bg-gradient-to-r from-fuchsia-500 to-purple-600 hover:from-fuchsia-400 hover:to-purple-500 text-white font-bold px-4 py-2 rounded-lg shadow-[0_0_15px_3px_rgba(236,72,153,0.4)] hover:scale-105 transform transition-all group">
                        <span class="relative z-10 font-mono tracking-wider text-sm">Reenviar Correo</span>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full underline text-sm text-purple-300 hover:text-white font-mono transition-all">
                        {{ __('Cerrar Sesión') }}
                    </button>
                </form>
            </div>
